...
readxl::read_xlsx("kirikou_annotations.xlsx", sheet = "folktales Kpelle children") |>
  ...
  summarize(text = str_c(txenfant, collapse = " "),
            ...
            time_end = max(time_end),
            duration = time_end - time_start) |> 
  mutate(age_group = "folktales (children)") |> 
  select(-time_start, -time_end) |> 
  ungroup() ->
  kpelle_chi_corpus
...
readxl::read_xlsx("kirikou_annotations.xlsx", sheet = "folktales Kpelle adults") |>
  ...
  summarize(text = str_c(txenfant, collapse = " "),
            ...
            time_end = max(time_end),
            duration = time_end - time_start) |> 
  mutate(age_group = "folktales (adults)") |> 
  select(-time_start, -time_end) |> 
  ungroup() |> 
...
people_text_color_k <- colors_m[c(rep(5, 2), rep(4, 5))]
```
...
The breakdown of constructions within the reported speech alone:
...
mano_corpus |> 
  ...
  mutate(chunk_structure = map_chr(mano_corpus$chunk_structure, extract_and_concatenate_rs),
         PST = str_count(chunk_structure, "PST"),
         JNT = str_count(chunk_structure, "JNT"),
         IPFV = str_count(chunk_structure, "IPFV"),
         PRED = str_count(chunk_structure, "PRED"),
         EXI = str_count(chunk_structure, "EXI"),
...
tense_k |> 
  ...
  select(-n, -total) |> 
  ungroup() |> 
  mutate(age2 = if_else(age > 20, "ad", as.character(age)),
         new_source = str_c(new_source, " (", age2, ")"),
         new_source = fct_reorder(new_source, age),
         tense = factor(tense, levels = c("CONDs", "PROH or IMP", "non-past", 
                                          "NEG", "CONJ", "CONJ_PST", "PRF", 
                                          "JNT or QUOT", "PST")),
         label = str_c(round(ratio * 100), "%")) |>
  filter(ratio > 0) |>
  ggplot(aes(x = ratio, y = new_source, label = label, color = tense)) +
  ...
  scale_x_continuous(label = scales::percent) +
  labs(x = NULL, y = NULL, fill = NULL) +
  theme(axis.text.y = element_text(colour = people_text_color_k)) +
  guides(color = "none")
```
...
kpelle_corpus |> 
  select(new_source, age, group_var, chunk_structure) |> 
  mutate(chunk_structure = map_chr(kpelle_corpus$chunk_structure, extract_and_concatenate_rs),
         CONJ_PST = str_count(chunk_structure, "CONJ/PST"),
         PST = str_count(chunk_structure, "PST")-CONJ_PST,
         CONJ = str_count(chunk_structure, "CONJ")-CONJ_PST,
         JNT = str_count(chunk_structure, "JNT"),
         QUOT = str_count(chunk_structure, "QUOT"),
...
mano_corpus |> 
  ...
  geom_col() +
  labs(y = NULL,
       x = "ratio of RES and RETR forms",
       fill = NULL)+
  scale_fill_manual(values = colors_m[c(4, 5, 1, 2, 3)]) +
...
kpelle_corpus |> 
  ...
  ggplot(aes(ratio, new_source, fill = group_var)) +
  geom_col() +
  scale_fill_manual(values = colors_m[c(4, 5)]) +
  labs(y = NULL,
       x = "ratio of RES and RETR forms",
       fill = NULL)
```
...
> Visualize subset of complex syntax: the ratio of  prefixes (temp, prec, cond, comme, tail-head, ht[, ht{, ht<NP.NMLZ> + glosses (COND, not the macrocategory but only the gloss) + "clause-final ā as the only marker of subordination" (search in the comment field, column AA) to all predications, without filtering RS
...
mano_corpus |> 
  ...
  mutate(temp = str_count(chunk_structure, "temp"),
         ...
         ht_np_nmlz = str_count(chunk_structure, "ht<NP.NMLZ>"),
         cond = str_count(chunk_structure, "COND"),
         clause_final_a = str_count(comment, "clause-final ā as the only marker of subordination"),
         sum_of_all = temp+prec+cond+comme+tail_head+ht+ht_np_nmlz+cond+clause_final_a,
         n_predecations = str_count(chunk_structure, "\\["),
...
kpelle_corpus |> 
  ...
  mutate(temp = str_count(chunk_structure, "temp"),
         ...
         sum_of_all = temp+comme+cond+tail_head+ht+ht_np_nmlz+cond+clause_final_i,
         n_predecations = str_count(chunk_structure, "\\["),
         ratio = sum_of_all/n_predecations) |> 
  select(-comment, -chunk_structure)
...
> check purp (prefix) + PURP + mod to all predications, without filtering
...
mano_corpus |> 
  ...
  mutate(purp_gloss = str_count(chunk_structure, "PURP"),
         ...
         n_predecations = str_count(chunk_structure, "\\["),
         ratio = (purp_gloss+purp_prefix+mod)/n_predecations) |> 
  arrange(ratio) |> 
  mutate(age2 = if_else(age == 100, "ad", as.character(age)),
         new_source = str_c(new_source, " (", age2, ")"),
         new_source = fct_inorder(new_source)) |> 
...
> how many tokens of COND in Mano outside RS?

```{r}
mano_corpus |> 
  mutate(chunk_structure = str_remove_all(chunk_structure, "rs%.*?%"),
         cond = str_count(chunk_structure, "COND")) |> 
  summarise(cond = sum(cond)) |> 
  pull(cond)
```

### Kpelle

> How many tokens of COND in Kpelle outside RS?
...
> Visualize ratios of jnt.say vs jnt

```{r}
mano_corpus |> 
  select(new_source, age, group_var, chunk_structure) |> 
  mutate(jnt_say = str_count(chunk_structure, "JNT\\.SAY"),
         jnt = str_count(chunk_structure, "JNT"),
         ratio = jnt_say/jnt) |> 
  select(-chunk_structure) |> 
  arrange(ratio) 
...
mano_corpus |> 
  ...
  mutate(jnt_say = str_count(chunk_structure, "JNT\\.SAY"),
         jnt = str_count(chunk_structure, "JNT"),
         ratio = jnt_say/jnt,
         ratio = if_else(is.nan(ratio), 0, ratio)) |> 
  arrange(ratio) |> 
  mutate(age2 = if_else(age == 100, "ad", as.character(age)),
         new_source = str_c(new_source, " (", age2, ")"),
         new_source = fct_inorder(new_source)) |> 
...
mano_corpus |> 
  select(new_source, age, group_var, chunk_structure) |> 
  mutate(jnt_say = str_count(chunk_structure, "JNT\\.SAY"),
         say = str_count(chunk_structure, "\\.SAY"),
         ratio = jnt_say/say) |> 
  select(-chunk_structure) |> 
  arrange(ratio) 
...
mano_corpus |> 
  select(new_source, age, group_var, chunk_structure) |> 
  mutate(says = str_extract_all(chunk_structure, "_[A-Z]*?\\.SAY")) |> 
  select(-chunk_structure) |> 
  unnest_longer(says) |> 
  mutate(says = str_remove_all(says, "\\.SAY"),
         says = str_remove_all(says, "_")) |> 
  count(new_source, group_var, says) |> 
  pivot_wider(names_from = says, values_from = n, values_fill = 0) 


people_text_color_m_for_jnt_plot <- 
  c(
      colors_m[4], # s_7_t_1  folktales (adults)
      colors_m[4], # s_7_t_2  folktales (adults)
      colors_m[5], # s_14_t_1 folktales (children)
...
mano_corpus |> 
  ...
  mutate(says = str_remove_all(says, "\\.SAY"),
         says = str_remove_all(says, "_")) |> 
  count(new_source, age, group_var, says) |> 
  group_by(new_source, age, group_var) |> 
  mutate(total = sum(n),
         ratio = n/total) |> 
  select(-n, -total) |> 
  pivot_wider(names_from = says, values_from = ratio, values_fill = 0) |> 
  mutate(JNT_RATIO = JNT) |> 
  pivot_longer(names_to = "says", values_to = "ratio", JNT:PRF) |> 
  filter(ratio != 0) |> 
  ungroup() |> 
  mutate(age2 = if_else(age == 100, "ad", as.character(age)),
         new_source = str_c(new_source, " (", age2, ")"),
         new_source = fct_reorder(new_source, JNT_RATIO),
         label = str_c(round(ratio * 100), "%"),
         says = fct_relevel(says, "JNT", after = 6),
         says = fct_relevel(says, "IPFV", after = 5),
         says = fct_relevel(says, "PST", after = 4)) |>
  filter(ratio > 0) |>
  ggplot(aes(x = ratio, y = new_source, label = label, color = says)) +
  geom_col(aes(fill = says)) +
  geom_label(position = position_fill(vjust = 0.5), fill = "white") +
  scale_x_continuous(label = scales::percent) +
  labs(x = NULL, y = NULL, fill = NULL, color = NULL) +
  theme(axis.text.y = element_text(colour = people_text_color_m_for_jnt_plot)) +
  guides(color="none")
```
...
> Visualize ratios of quot + jnt.say vs jnt
...
> а, поняла, там надо наверное quot + jnt.say / say + quot

```{r}
kpelle_corpus |> 
  select(new_source, age, group_var, chunk_structure) |> 
  mutate(jnt_say = str_count(chunk_structure, "JNT\\.SAY"),
         says = str_count(chunk_structure, "SAY"),
         quot = str_count(chunk_structure, "QUOT"),
         ratio = (quot+jnt_say) /(says + quot),
         ratio = if_else(is.nan(ratio), 0, ratio)) |> 
  select(-chunk_structure) |> 
  arrange(ratio) 
...
kpelle_corpus |> 
  select(new_source, age, group_var, chunk_structure) |> 
  mutate(says = str_extract_all(chunk_structure, "(_[A-Z]*?\\.SAY)|(QUOT)")) |> 
  select(-chunk_structure) |> 
  unnest_longer(says) |> 
  mutate(says = str_remove_all(says, "\\.SAY"),
         says = str_remove_all(says, "_")) |> 
...
kpelle_corpus |> 
  ...
  ungroup() |> 
  pivot_wider(names_from = says, values_from = ratio, values_fill = 0) |> 
  mutate(QUOT_RATIO = QUOT) |> 
  pivot_longer(names_to = "says", values_to = "ratio", QUOT:COND) |> 
  filter(ratio != 0) |> 
  mutate(age2 = if_else(age > 20, "ad", as.character(age)),
         new_source = str_c(new_source, " (", age2, ")"),
         new_source = fct_reorder(new_source, QUOT_RATIO),
         label = str_c(round(ratio * 100), "%"),
         says = fct_relevel(says, "JNT", after = 8),
         says = fct_relevel(says, "QUOT", after = 7)) |>
  filter(ratio > 0) |>
  ggplot(aes(x = ratio, y = new_source, label = label, color = says)) +
  geom_col(aes(fill = says)) +
  geom_label(position = position_fill(vjust = 0.5), fill = "white") +
  ...
  labs(x = NULL, y = NULL, fill = NULL, color = NULL) +
  guides(color="none") +
  theme(axis.text.y = element_text(colour = people_text_color_k[c(1, 3, 3, 3, 3, 3, 1)]))
```
...
mano_corpus |> 
  select(new_source, age, group_var, chunk_structure) |> 
  mutate(total = str_count(chunk_structure, prefix),
         predecations_total = str_count(chunk_structure, "\\["),
         cs_ors = str_remove_all(chunk_structure, "rs%.*?%"),
         predecations_ors = str_count(cs_ors, "\\["),
         predecations_wrs = predecations_total - predecations_ors,
         ors = str_count(cs_ors, prefix),
         wrs = total - ors,
         total = total/predecations_total,
         ors = ors/predecations_ors,
         wrs = wrs/predecations_wrs,
         total = if_else(is.nan(total), 0, total),
         ors = if_else(is.nan(ors), 0, ors),
         wrs = if_else(is.nan(wrs), 0, wrs)) |> 
  select(-chunk_structure, -cs_ors, -predecations_total, -predecations_ors, -predecations_wrs) |> 
  pivot_longer(names_to = "type", values_to = "value", total:wrs) |> 
  mutate(type = case_when(type == "total" ~ str_c("overall ratio of ", prefix),
                          type == "ors" ~ str_c("ratio of ", prefix, " outside RS"),
                          type == "wrs" ~ str_c("ratio of ", prefix, " within RS"))) |> 
  mutate(age2 = if_else(age == 100, "ad", as.character(age)),
         new_source = str_c(new_source, " (", age2, ")"),
         new_source = reorder_within(new_source, within = type, by = value)) |> 
  ggplot(aes(value, new_source, fill = group_var))+
  geom_col()+
  facet_wrap(~type, nrow = 1, scales = "free")+
  scale_y_reordered() +
  labs(y = NULL, x = "ratio", fill = NULL) +
  scale_fill_manual(values = colors_m[c(4, 5, 1, 2, 3)]) +
  guides(fill=guide_legend(nrow=2, byrow=TRUE))
...
## comme + prec + cond + temp 

### Mano 

> comme + prec + cond + temp  outside RS in Mano
...
> comme + cond + temp2 in Kpelle

```{r}
kpelle_corpus |> 
  select(new_source, age, group_var, chunk_structure, comment) |> 
  mutate(chunk_structure = str_remove_all(chunk_structure, "rs%.*?%"),
         temp2 = str_count(chunk_structure, "temp2"),
         cond = str_count(chunk_structure, "cond"),
         comme = str_count(chunk_structure, "comme"),
         total = comme  + cond + temp2) |> 
  summarise(total = sum(total)) |> 
  pull(total)
```
