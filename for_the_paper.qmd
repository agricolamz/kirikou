...
mano_corpus |> 
  ...
  mutate(jnt_say = str_count(chunk_structure, "JNT\\.SAY"),
         jnt = str_count(chunk_structure, "JNT")-jnt_say,
         ratio = jnt/(jnt+jnt_say)) |> 
  arrange(ratio) 
...
mano_corpus |> 
  select(new_source, age, group_var, chunk_structure) |> 
  mutate(jnt_say = str_count(chunk_structure, "JNT\\.SAY"),
         say = str_count(chunk_structure, "\\.SAY")-jnt_say,
         ratio = jnt_say/(say+jnt_say)) |> 
  arrange(ratio) 

mano_corpus |> 
  select(new_source, age, group_var, chunk_structure) |> 
  mutate(jnt_say = str_count(chunk_structure, "JNT\\.SAY"),
         other_say = str_count(chunk_structure, "\\.SAY")-jnt_say,
         ratio = jnt_say/(other_say+jnt_say),
         ratio = if_else(is.nan(ratio), 0, ratio)) |> 
  arrange(ratio) |> 
  mutate(age2 = if_else(age == 100, "ad", as.character(age)),
         new_source = str_c(new_source, " (", age2, ")"),
...
kpelle_corpus |> 
  select(new_source, age, group_var, chunk_structure) |> 
  mutate(jnt_say = str_count(chunk_structure, "JNT\\.SAY"),
         says = str_count(chunk_structure, "SAY") - jnt_say,
         quot = str_count(chunk_structure, "quot"),
         ratio = (quot+jnt_say) /(quot+jnt_say+says),
         ratio = if_else(is.nan(ratio), 0, ratio)) |> 
  arrange(ratio) 
