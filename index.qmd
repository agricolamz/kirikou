...
readxl::read_xlsx("kirikou_annotations.xlsx", sheet = "folktales Mano adults") |> 
  ...
  mutate(group_var = case_when(text_type == "kirikou" & residence_place == "Nzerekore" ~ "Kirikou (Nzerekore)",
...
mano_corpus |> 
  ...
  ggplot(aes(n, word))+
  geom_col()+
  facet_wrap(~group_var, scales = "free", nrow = 1)+
  scale_y_reordered()+
  labs(x = NULL, y = NULL)
```
...
mano_corpus |> 
  mutate(name = str_c(name, " (", age, "):\t", group_var),
         color = scales::hue_pal()(4)[factor(group_var)]) |> 
  distinct(name, color) ->
  colors
...
tense_marking |> 
  ...
  count(name, age, group_var, chunk_structure, sort = TRUE) |> 
...
tense_marking |> 
  mutate(pst_jnt_other = if_else(str_detect(chunk_structure, "(PST)|(JNT)"), 
                                 "PST_or_JNT",
                                 "other")) |> 
  count(name, age, group_var, pst_jnt_other, sort = TRUE) |> 
