...
percantage_ors <- round((pst_jnt_ors$pst + pst_jnt_ors$jnt)/n_predecations$n_predecations*100)
```

By excluding reported speech we arrive at an even **LOWER** proportion: `r pst_jnt_ors$pst` and `r pst_jnt_ors$jnt` tokens of PST and JNT, respectively, representing `r pst_jnt_ors$pst+pst_jnt_ors$jnt` and `r percantage_ors`% of predications.
