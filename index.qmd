...
The total duration of the corpus is 2 h 50 min, while the median duration of texts is 4 min.
...
percantage <- round((pst_jnt$pst + pst_jnt$jnt)/n_predecations$n_predecations*100)
```

Overall, in our (Mano) corpus they were used `r pst_jnt$pst` and `r pst_jnt$jnt` times respectively, or  and `r percantage`%, out of `r n_predecations$n_predecations` finite predications total.
...
mano_corpus |> 
  ...
  mutate(ors = str_remove_all(chunk_structure, "rs%.*?%"),
         pst = str_count(ors, "PST"),
         jnt = str_count(ors, "JNT"),
         n_predicates = str_count(ors, "\\[")) |> 
  select(-chunk_structure, -ors) |> 
  summarise(pst = sum(pst),
            jnt = sum(jnt),
            n_predicates = sum(n_predicates))->
  pst_jnt_ors

percantage_ors <- round((pst_jnt_ors$pst + pst_jnt_ors$jnt)/pst_jnt_ors$n_predicates*100)
```

By excluding reported speech we arrive at an even higher proportion: `r pst_jnt_ors$pst` and `r pst_jnt_ors$jnt` tokens of PST and JNT, respectively, representing `r pst_jnt_ors$pst+pst_jnt_ors$jnt` and `r percantage_ors`% of predications.

> Is it true that IMP is only used in RS? what about PROH?
...
There are only `r imp_proh$imp_ors` utterances of IMP outside of RS out of `r imp_proh$imp` total utterances. There are only `r imp_proh$proh_ors` utterances of PROH outside of RS out of `r imp_proh$proh` total utterances.

> Visualization of RES + PHAS + RETR to the number of predications outside RS
...
percantage_res <- round((res_phas_retr$res)/n_predecations$n_predecations*100)
percantage_phas <- round((res_phas_retr$phas)/n_predecations$n_predecations*100)
percantage_retr <- round((res_phas_retr$retr)/n_predecations$n_predecations*100)

res_phas_retr_sum <- sum(res_phas_retr)
percantage_res_phas_retr <- round((res_phas_retr_sum)/n_predecations$n_predecations*100)
```

- There are only `r res_phas_retr_sum` utterances of RES outside of RS (`r percantage_res_phas_retr`%). 
- There are only `r res_phas_retr$res` utterances of RES outside of RS (`r percantage_res`%). 
- There are only `r res_phas_retr$phas` utterances of PHAS outside of RS (`r percantage_phas`%). 
- There are only `r res_phas_retr$retr` utterances of RETR outside of RS (`r percantage_retr`%).

Я не очень понимаю, о какой визуализации ты говоришь... Там такие крошечные числа...
...
> calculate and visualize individual ratios of DUR to text length in predications outside RS
...
percantage_dur <- round(dur/n_predecations$n_predecations*100)
```

- There are only `r dur` utterances of RES outside of RS (`r percantage_dur`%). 
...
mano_corpus |> 
  ...
  mutate(chunk_structure = str_remove_all(chunk_structure, "rs%.*?%"),
         dur = str_count(chunk_structure, "DUR"),
         n_predecations = str_count(chunk_structure, "\\[")) |> 
  ggplot(aes(n_predecations, dur, color = group_var))+
  geom_smooth(method = "lm", se = FALSE, color = "grey60", linetype = 2)+
  geom_point()
...
> visualize which TAMP constructions are used with DUR (check all forms that have the _DUR extension, such as PST_DUR, separate DUR and regroup the remainder of the forms in the same way as you did when visualizing tense diversity)

```{r}
mano_corpus |> 
  select(name, age, group_var, chunk_structure) |> 
  mutate(chunk_structure = str_remove_all(chunk_structure, "rs%.*?%"),
         dur = str_extract_all(chunk_structure, "_[A-Z]*?_DUR")) |> 
  select(-chunk_structure) |> 
  unnest_longer(dur) |> 
  mutate(dur = str_remove_all(dur, "_DUR"),
         dur = str_remove_all(dur, "_")) |> 
  count(name, age, group_var, dur) |> 
  pivot_wider(names_from = dur, values_from = n, values_fill = 0)
...
mano_corpus |> 
  ...
  mutate(dur = str_remove_all(dur, "_DUR"),
         dur = str_remove_all(dur, "_")) |> 
  count(name, age, group_var, dur) |> 
  group_by(name, age, group_var, dur) |>
  summarise(n = sum(n)) |> 
  group_by(name, age, group_var) |> 
  mutate(total = sum(n),
         ratio = n/total) |> 
  ...
  mutate(name = str_c(name, " (", age, ")"),
         ...
         label = str_c(round(ratio * 100), "%")) |>
  filter(ratio > 0) |>
  ggplot(aes(x = ratio, y = name, label = label, color = dur))+
  geom_col(aes(fill = dur))+
  geom_label(position = position_fill(vjust = 0.5), fill = "white")+
  scale_x_continuous(label = scales::percent)+
  labs(x = NULL, y = NULL)
```

Oh, it is Barbie time...

> visualize subset of complex syntax: prefixes (temp, prec, cond, comme, tail-head), + glosses (COND) + "clause-final ā as the only marker of subordination" (search in the comment field), without filtering RS


> check purp (prefix) + PURP to all predications, without filtering

> Kpelle: quot vs jnt.say in Kpelle
> jnt vs quot + jnt.say in Kpelle
> quot+jnt.say vs other.say in Kpelle
